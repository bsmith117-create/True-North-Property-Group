...
import ResourcesHeaderImg from '../../media/ResourcesHeader.webp';
...
const calculateQualifyingLoan = (income: number, monthlyHousingBudget: number, rate: number, term: number) => {
    if (income <= 0 || monthlyHousingBudget <= 0) return 0;
    ...
    return monthlyHousingBudget * (numerator / denominator);
};
...
const FinancialAssessmentPage: React.FC = () => {
    ...
    // Derived values (computed during render, no state sync needed)
    ...
    const totalDebt = safeCarPayment + safeCreditCardPayment + safeStudentLoan + safeOtherDebt;
    const maxMonthlyPaymentForHousing = safeIncome > 0 ? (safeIncome * targetDti) - totalDebt : 0;
    const monthlyPayment = maxMonthlyPaymentForHousing > 0 ? maxMonthlyPaymentForHousing : 0;
    const dti = safeIncome > 0 ? (totalDebt / safeIncome) * 100 : 0;
    const qualifyingLoan = calculateQualifyingLoan(safeIncome, maxMonthlyPaymentForHousing, assumedRate, assumedTerm);
